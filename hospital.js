...
class Hospital {
    ...
    rl.question('\nPilih Menu: ', (input) => {
      switch (input.split(' ')[0]) {
        ...
        case '2':
          ...
          break
        case '3':
          this.lihatRekamMedis(input.split(' ')[1]);
          break
        case '4':
          this.tambahPasien();
          break
        case '5':
          this.hapusDataPasien(input.split(' ')[1]);
          break
        case '6':
          this.tambahEmployee();
          break
        case '7':
          ...
          break
        case '8':
          this.hapusDataEmployee(input.split(' ')[1]);
          break
      }
    })
  }
  ...
  start() {
    ...
    console.log('----------------------------------');
    rl.question(`\nLogin as: \n${chalk.red('[1] Infected')} \n${chalk.blue('[2] Maesters')} \n\n Insert Here: `, (input) => {
      if (input === '1') {
        this.loginAsPatient()
      }
      if (input === '2') {
  ...
  loginAsPatient() {
    ...
    rl.question('Nama: \n', (input) => {
      this.dataPasien.forEach((data) => {
        if (data.name.toUpperCase() === input.toUpperCase()) {
...
let hospital = new Hospital('The Citadel', '\nOld Town\n')
hospital.tambahPasien(new Patient(1, 'Ser Jorah', 'Stoneman Disease'))

hospital.tambahEmployee(new Employee('Archmaester Armand', 'Admin', 'arch', 'maester'))
hospital.tambahEmployee(new Employee('Maester Aegon', 'Dokter', 'maester', 'aegon'))
hospital.tambahEmployee(new Employee('Samwell Tarly', 'Office Boy', 'sam', 'sam'))
hospital.start()